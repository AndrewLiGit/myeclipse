...
如果在hibernate中把事务隔离级别设置为serializable,那么我们就完全没有必要使用hibernate中的锁机制了,但是这样设置后事务的执行效率很低,所以我们一般会设置为read-committed(一般默认也是这个),这样既能解决脏读又有一定的效率,只是在这种设置下仍然会有不可重复读的问题(幻读不考虑,很少出现),所以我们在hibernate就可以使用锁机制来解决这个问题.
